...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            max_distance = distances.min(axis = 1)

            index = max_distance.idxmax()
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if isinstance(point1, pd.Series) and isinstance(point2, pd.Series):
            return np.sqrt(np.sum((point1.astype(float) - point2.astype(float)) ** 2))
        #DataFrame and Series
        elif isinstance(point1, pd.DataFrame) and isinstance(point2, pd.Series):
            return np.sqrt(((point1.astype(float) - point2.astype(float)) ** 2).sum(axis = 1))
        else:
            raise ValueError("Invalid input types. Expected Series or DataFrame.")
    ...
    def adjust_centroids(self, data, groups):
        ...
        centroids = grouped_data.groupby('group')[num_cols].mean()
